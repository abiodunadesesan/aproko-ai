...
export async function installWorkspaceMocks(page: Page): Promise<void> {
  ...
  await page.route(new RegExp(`/api/v1/workspaces/${WORKSPACE_ID}/projects$`), async (route) => {
    if (route.request().method() === 'GET') {
  ...
  await page.route(
    new RegExp(`/api/v1/workspaces/${WORKSPACE_ID}/projects/[^/]+/folders`),
    async (route) => {
      if (route.request().method() === 'GET') {
  ...
  await page.route(new RegExp(`/api/v1/workspaces/${WORKSPACE_ID}/sources`), async (route) => {
    if (route.request().method() === 'GET') {
  ...
  await page.route(
    new RegExp(`/api/v1/workspaces/${WORKSPACE_ID}/chat/sessions`),
    async (route) => {
